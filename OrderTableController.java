...
//        productSearchController.setOrderButton();
        String query = "select products.pid,products.name,products.price,orders.ts from orders inner join products on orders.product_id = products.pid where customer_id = "+Controller.loggedInCustomer.getId()+";";
...
//        id.setCellValueFactory(new PropertyValueFactory("pid"));
        ...
        productList = getProducts(query);
        orderTable.setItems(productList);
        orderTable.getColumns().addAll(name, price,time);
    ...
    public static ObservableList<Product> getProducts(String query) {
        ...
        try {
            if (rs != null) {
                while (rs.next()) {
                    result.add(new Product(rs.getInt("pid"), rs.getString("name"), rs.getDouble("price"),rs.getTimestamp("ts")));
